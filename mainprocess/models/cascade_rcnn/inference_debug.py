...
def setup_model():
    # TODO: the model and cfg should be set here...
    pass

# Convert instances to dictionary for saving


def run_inference(image_path, output_dir, cfg):
    predictor = DefaultPredictor(cfg)

    # load image 
    image = cv2.imread(image_path)
    if image is None:
        raise Exception(f"Error: cannot load image {image_path}")
        return
...
if __name__ == "__main__":
    ...
    run_inference('samples/Geoservice.png', 'sample', cfg)