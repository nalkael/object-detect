...
def run_sahi_inference(image_path, model_weights_path='yolo8x.pt', output_dir=None, overlap=0.3, confidence_threshold=0.5, image_size=640):
    ...
    result_filename = os.path.splitext(os.path.basename(image_path))[0] + "_results.json"
    json_path = os.path.join(output_dir, result_filename)
    with open(json_path, "w") as f:
        json.dump(result_json, f, indent=4)

    # Save visualization
    image_pred_visual = os.path.splitext(os.path.basename(image_path))[0] + "_predict_yolov8.png"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run YOLOv8 object detection with SAHI slicing.")
    ...
    run_inference(args.image_path, args.model_weights, args.output_dir, args.overlap, args.conf, args.img_size)
