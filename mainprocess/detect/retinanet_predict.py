...
def run_sahi_inference(image_path, model_config_path, model_weights_path, output_dir=None, overlap=0.3, conf=0.5, image_size=640):
    ...
    model_config_path = "trained_models/retina_net/model_config.yaml"
    model_weights_path = "trained_models/retina_net/best_model.pth"
    ...
    result_filename = os.path.splitext(os.path.basename(image_path))[0] + "_results_retinanet.json"
    json_path = os.path.join(output_dir, result_filename)
    with open(json_path, "w") as f:
        json.dump(result_json, f, indent=4)

    # Save visualization
    image_pred_visual = os.path.splitext(os.path.basename(image_path))[0] + "_predict_retinanet"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run RetinaNet object detection model with SAHI slicing.")
    parser.add_argument("image_path", type=str, help="Path to input image.")
    parser.add_argument("model_config", type=str, help="Path to RetinaNet model configs.")
    parser.add_argument("model_weights", type=str, help="Path to RetinaNet model weights.")
